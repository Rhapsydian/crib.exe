 ...
 *
 * Two independent accumulation sources: a lost regular/elite duel
 * (heatFromLoss, session 9's margin-of-loss formula) and a flat cost per
 * map move (HEAT_PER_MOVE, session 19's free-roam pressure). Gatekeeper
 * losses bypass Heat entirely -- they go straight to Quarantine -- so
 * heatFromLoss only ever applies to regular/elite tiers.
 */
...
/**
 * Heat gained from losing a regular/elite duel, scaled by how close the
 * player got to their own win before being dragged back -- not literal
 * overshoot (Breach/Containment stops dead at 0/100), but the peak value
 * reached during the match (see CombatResult.peakBreachContainment).
 * Getting the meter to 80% in your favor before it swung costs
 * noticeably less than being dominated from the first hand.
 */
